...
class DataSubjectRequestService:
    ...
    def create_request(
        ...
        AuditLog.log_dsr(
            action=(
                AuditLog.ACTION_DSR_ACCESS
                if request_type == self.REQUEST_ACCESS
                else (
                    AuditLog.ACTION_DSR_ERASE
                    if request_type == self.REQUEST_ERASE
                    else (
                        AuditLog.ACTION_DSR_RECTIFY
                        if request_type == self.REQUEST_RECTIFY
                        else (
                            AuditLog.ACTION_DSR_RESTRICT
                            if request_type == self.REQUEST_RESTRICT
                            else (
                                AuditLog.ACTION_DSR_PORTABILITY
                                if request_type == self.REQUEST_PORTABILITY
    ...
    def process_rectification_request(
        ...
        AuditLog.log_dsr(
            ...
            extra={
                "request_id": request_id,
                "fields_updated": [f["field"] for f in updated_fields],
            },
        )
