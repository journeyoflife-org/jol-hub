...
class AuditLog(UUIDModel, TimeStampedModel):
    ...
    action = models.CharField(
        _("action"), max_length=24, choices=ACTION_CHOICES, db_index=True
    )
    ...
    correlation_id = models.UUIDField(
        _("correlation ID"), null=True, blank=True, db_index=True
    )
    ...
    consent_reference = models.CharField(
        _("consent reference"), max_length=64, blank=True
    )
    legal_basis = models.CharField(_("legal basis"), max_length=32, blank=True)
    data_subject_id = models.CharField(
        _("data subject ID"), max_length=128, blank=True, db_index=True
    )
    ...
    def __str__(self):
        return (
            f"{self.action} {self.entity_type}({self.entity_id}) by user {self.user_id}"
        )
