...
class ContactAdmin(admin.ModelAdmin):
    ...
    def grant_consent(self, request, queryset):
        ...
        self.message_user(
            request, f"Granted consent for {updated} contacts.", messages.SUCCESS
        )
    ...
    def withdraw_consent(self, request, queryset):
        ...
        self.message_user(
            request, f"Withdrew consent for {updated} contacts.", messages.WARNING
        )
    ...
    def sync_to_bitrix24(self, request, queryset):
        ...
        self.message_user(
            request, f"Queued {count} contacts for Bitrix24 sync.", messages.INFO
        )
...
class DealAdmin(admin.ModelAdmin):
    ...
    def mark_paid(self, request, queryset):
        ...
        self.message_user(request, f"Marked {updated} deals as paid.", messages.SUCCESS)

    mark_paid.short_description = _("Mark as paid")

    def send_receipts(self, request, queryset):
        """Send receipts for paid deals."""
        to_send = queryset.filter(stage=Deal.DealStage.PAID, receipt_sent=False)
        updated = to_send.update(
            receipt_sent=True,
            receipt_sent_at=timezone.now(),
        )
        self.message_user(
            request, f"Queued {updated} receipts for sending.", messages.INFO
        )
...
class AuditEntryAdmin(admin.ModelAdmin):
    ...
    def verify_integrity(self, request, queryset):
        ...
        for org_id in org_ids:
            ...
            results.append(
                f"Org {org_id}: {'OK' if result['valid'] else 'COMPROMISED'}"
            )

        self.message_user(
            request, "Integrity check results:\n" + "\n".join(results), messages.INFO
        )
...
class DataSubjectRequestAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ["id", "due_date", "completed_at", "created_at", "updated_at"]
    ...
    def mark_completed(self, request, queryset):
        ...
        self.message_user(
            request, f"Completed {queryset.count()} requests.", messages.SUCCESS
        )
