...
AUDIT_ENTRIES = Counter(
    "jolhub_audit_entries_total", "Audit log entries", ["tenant_id", "event_type"]
)

AUDIT_INTEGRITY_CHECKS = Gauge(
    "jolhub_audit_integrity_checks", "Audit integrity check results", ["tenant_id"]
)
...
class ComplianceMonitor:
    ...
    def generate_report(cls, tenant_id: Optional[str] = None) -> ComplianceReport:
        ...
        report = ComplianceReport(tenant_id=tenant_id, generated_at=timezone.now())
        ...
        report.overdue_dsr_count = dsr_qs.filter(
            status=DataSubjectRequest.Status.PENDING, due_date__lt=timezone.now().date()
        ).count()

        completed_dsrs = dsr_qs.filter(
            status=DataSubjectRequest.Status.COMPLETED, completed_at__isnull=False
        )
        if completed_dsrs.exists():
            # Calculate average response time
            total_days = sum(
                (dsr.completed_at - dsr.created_at).days for dsr in completed_dsrs
            )
        ...
        # Legal holds
        report.active_legal_holds = contact_qs.filter(legal_hold=True).count()

        # Audit metrics
        audit_qs = AuditEntry.objects.filter(organization_id=tenant_id)
        integrity_result = AuditEntry.verify_chain(tenant_id)
        report.audit_integrity_status = (
            "ok" if integrity_result["valid"] else "compromised"
        )
    ...
    def check_bitrix24(tenant_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if circuit_breaker.is_open:
            return {
                ...
                "next_retry": (
                    circuit_breaker.next_retry_time.isoformat()
                    if circuit_breaker.next_retry_time
                    else None
                ),
            }
