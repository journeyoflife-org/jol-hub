...
class Command(BaseCommand):
    ...
    def _sync_contacts(
        ...
        for contact in queryset[:batch_size]:
            if dry_run:
                self.stdout.write(
                    f"  Would sync: {contact.full_name} (ID: {contact.id})"
                )
                results["skipped"] += 1
                continue
            ...
            try:
                ...
                if result.success:
                    ...
                    results["failed"] += 1
                    self.stderr.write(f"  Failed: {contact.full_name} - {result.error}")
        ...
        for deal in queryset[:batch_size]:
            ...
            try:
                ...
                if result.success:
                    ...
                    results["failed"] += 1
                    self.stderr.write(f"  Failed: {deal.deal_number} - {result.error}")
            except Exception as e:
                results["failed"] += 1
    ...
    def _print_summary(self, results: dict):
        ...
        if total_failed == 0:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Completed: {total_synced} entities synced successfully"
                )
            )
        else:
            self.stdout.write(
