...
class Command(BaseCommand):
    ...
    def _check_tenant(self, tenant_id: str, alert_thresholds: bool, output_format: str):
        ...
        if report.compliance_score < 50:
            raise CommandError(
                f"Critical compliance issues detected (score: {report.compliance_score})"
            )
    ...
    def _print_report(self, report: ComplianceReport):
        ...
        self.stdout.write(
            f"  Avg response time:    {report.avg_dsr_response_days:.1f} days"
        )
        ...
        if report.compliance_score >= 90:
            self.stdout.write(
                self.style.SUCCESS(
                    f"STATUS: EXCELLENT ({report.compliance_score:.0f}%)"
                )
            )
        elif report.compliance_score >= 70:
            self.stdout.write(
                self.style.SUCCESS(f"STATUS: GOOD ({report.compliance_score:.0f}%)")
            )
        elif report.compliance_score >= 50:
            self.stdout.write(
                self.style.WARNING(
                    f"STATUS: NEEDS ATTENTION ({report.compliance_score:.0f}%)"
                )
            )
        else:
            self.stdout.write(
                self.style.ERROR(f"STATUS: CRITICAL ({report.compliance_score:.0f}%)")
            )
        self.stdout.write("=" * 60 + "\n")
