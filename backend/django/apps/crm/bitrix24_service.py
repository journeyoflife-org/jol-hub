...
from integrations.bitrix24.client import (Bitrix24ApiError, Bitrix24AuthError,
                                          Bitrix24Error,
    ...
    def _load_tenant_config(cls, tenant_id: str) -> Optional[Bitrix24Config]:
        ...
        try:
            ...
            if org.bitrix24_webhook_url:
                ...
                match = re.search(
                    r"/rest/(\d+)/([a-zA-Z0-9]+)/?$", org.bitrix24_webhook_url
                )
                if match:
        ...
        try:
            ...
            if contact.bitrix24_id:
                ...
                result = await self.client.contacts.update(
                    id=contact.bitrix24_id, fields=bitrix24_data
                )
            ...
            return SyncResult(success=True, entity=contact, bitrix24_id=bitrix24_id)

        except Bitrix24RateLimitError as e:
            self._handle_failure(e)
            return SyncResult(
                success=False, entity=contact, error="Rate limit exceeded"
            )
        except Bitrix24AuthError as e:
            self._handle_failure(e)
            return SyncResult(
                success=False, entity=contact, error="Authentication failed"
            )
        except Bitrix24ApiError as e:
            self._handle_failure(e)
            return SyncResult(
                success=False, entity=contact, error=f"API error: {e.message}"
            )
        except Exception as e:
            self._handle_failure(e)
            return SyncResult(success=False, entity=contact, error=str(e))
        ...
        try:
            ...
            if deal.bitrix24_id:
                ...
                result = await self.client.deals.update(
                    id=deal.bitrix24_id, fields=bitrix24_data
                )
            ...
            return SyncResult(success=True, entity=deal, bitrix24_id=bitrix24_id)

        except Bitrix24Error as e:
            self._handle_failure(e)
            return SyncResult(success=False, entity=deal, error=str(e))
...
class CRMBitrix24ServiceSync:
    ...
    def sync_contact(self, contact: Contact, **kwargs) -> SyncResult[Contact]:
        ...
        return self._run(
            self._async_service.sync_contact_to_bitrix24(contact, **kwargs)
        )
