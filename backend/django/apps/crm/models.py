...
class CRMTenantModel(BaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.organization_id:
            raise ValidationError("organization_id is required for tenant isolation")
    ...
    def grant_consent(self, version: str = "1.0"):
        ...
        self.save(
            update_fields=["consent_status", "consent_granted_at", "consent_version"]
        )
    ...
    def withdraw_consent(self):
        ...
        self.save(update_fields=["consent_status", "consent_withdrawn_at"])
...
class Contact(CRMTenantModel):
    ...
    first_communion_date = models.DateField(
        _("first communion date"), null=True, blank=True
    )
    ...
    def clean(self):
        ...
        if (
            ...
            and self.data_classification != DataClassification.SPECIAL_CATEGORY
        ):
...
class AuditEntry(BaseModel):
    ...
    def _get_next_sequence(self) -> int:
        ...
        last_entry = (
            AuditEntry.objects.filter(organization=self.organization)
    ...
    def verify_chain(cls, organization_id: int) -> Dict[str, Any]:
        ...
        entries = cls.objects.filter(organization_id=organization_id).order_by(
            "sequence_number"
        )
...
class DataSubjectRequest(BaseModel):
    ...
    def save(self, *args, **kwargs):
        if not self.due_date:
            ...
            self.due_date = (django_timezone.now() + timedelta(days=30)).date()
