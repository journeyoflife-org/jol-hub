...
class ContactViewSet(TenantIsolatedViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if has_religious:
            queryset = queryset.filter(~Q(religious_affiliation="")).exclude(
                religious_affiliation__isnull=True
            )
...
class DealViewSet(TenantIsolatedViewSetMixin, viewsets.ModelViewSet):
    ...
    @extend_schema(
        description="Process refund for deal", responses={200: DealSerializer}
    )
    @action(detail=True, methods=["post"], throttle_classes=[FinancialThrottle])
    ...
    @extend_schema(description="Send donation receipt", responses={200: DealSerializer})
    @action(detail=True, methods=["post"])
    def send_receipt(self, request, pk=None):
        ...
        if deal.receipt_sent:
            return Response(
                {"error": "Receipt already sent"}, status=status.HTTP_400_BAD_REQUEST
            )
...
@extend_schema(
    tags=["CRM - Audit"], description="Read-only audit log access for compliance."
)
class AuditEntryViewSet(TenantIsolatedViewSetMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def statistics(self, request):
        ...
        # Security events
        security_events = (
            queryset.filter(event_type__in=["access", "delete", "gdpr_request"])
        ...
        financial_events = (
            queryset.filter(event_type="financial_transaction")
    ...
    def chain_info(self, request):
        ...
        first_entry = (
            AuditEntry.objects.filter(organization_id=tenant_id)
        ...
        total_entries = AuditEntry.objects.filter(organization_id=tenant_id).count()
        ...
        return Response(
            {
                ...
                "last_entry": {
                    ...
                    "created_at": (
                        last_entry.created_at.isoformat() if last_entry else None
                    ),
                },
            }
        )
...
@extend_schema(
    tags=["CRM - Bitrix24"], description="Bitrix24 CRM synchronization operations."
)
