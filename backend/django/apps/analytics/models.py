...
class PageView(UUIDModel, TimeStampedModel):
    ...
    session_id = models.CharField(
        _("session ID"), max_length=64, blank=True, db_index=True
    )
    ...
    duration_seconds = models.PositiveIntegerField(
        _("duration (s)"), null=True, blank=True
    )
...
class DailyStats(UUIDModel, TimeStampedModel):
    ...
    bounce_rate = models.DecimalField(
        _("bounce rate"), max_digits=5, decimal_places=2, default=0
    )
    avg_session_duration = models.PositiveIntegerField(
        _("avg session duration (s)"), default=0
    )
