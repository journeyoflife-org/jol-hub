...
def validate_date_range(
    start: date, end: date, max_days: int = MAX_DATE_RANGE_DAYS
) -> None:
    """
...
def validate_positive_int(
    value: str, param_name: str, default: int, max_value: int = 100
) -> int:
    """
...
class AnalyticsOverviewView(APIView):
    ...
    def get(self, request):
        ...
        try:
            org_id = validate_organization_id(
            ...
            start = validate_date_param(
                ...
                request.query_params.get("end_date"), "end_date", default=date.today()
            )
...
class TopParishesView(APIView):
    ...
    def _get_organizations_with_consent(self):
        ...
        return list(
            ConsentSettings.objects.filter(analytics_consent_enabled=True).values_list(
