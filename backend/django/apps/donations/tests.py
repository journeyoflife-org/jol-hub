...
class TestDonationRefundAuditLogging:
    ...
    def test_refund_audit_records_when(self, db, factory, user, donation):
        ...
        assert (
            before_refund <= audit_entry.created_at.replace(tzinfo=None) <= after_refund
        )
    ...
    def test_refund_audit_records_what_changed(self, db, factory, user, donation):
        ...
        assert (
            "status" in details["field_changes"]
        ), "WHAT: status change must be recorded"
        ...
        assert (
            details["refund_reason"] == "Duplicate charge"
        ), "WHAT: refund_reason must be recorded"

    def test_refund_audit_has_tamper_evident_hash_chain(
        self, db, factory, user, donation
    ):
        """
        ...
        assert (
            len(audit_entry.entry_hash) == 64
        ), "Hash chain: entry_hash must be SHA-256 (64 chars)"
        assert (
            audit_entry.previous_hash is not None
        ), "Hash chain: previous_hash must exist"
        assert (
            audit_entry.sequence_number > 0
        ), "Hash chain: sequence_number must be positive"

    def test_refund_audit_chain_verification(
        self, db, factory, user, donation, organization
    ):
        """
        ...
        assert result[
            "valid"
        ], f"Hash chain verification failed: {result.get('errors', [])}"
    ...
    def test_refund_atomic_with_audit(self, db, factory, user, donation):
        ...
        with patch.object(
            AuditEntry.objects, "create", side_effect=Exception("DB error")
        ):
            try:
                response = view.post(request, donation.id)
            except Exception:
    ...
    def test_refund_audit_pci_dss_compliance(self, db, factory, user, donation):
        ...
        assert (
            audit_entry.actor_user is not None
        ), "PCI-DSS: Must log user who performed action"
        ...
        assert (
            audit_entry.event_type == AuditEntry.EventType.FINANCIAL_TRANSACTION
