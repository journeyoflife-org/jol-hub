 ...
 *
 * Content (from PageConfig): optional `title`, `subtitle`, `ctaButtons`, and
 * `variant` (STEP 7 — explicit vertical treatment; when omitted the Hero
...
/** Narrow an unknown config value to a valid HeroVariant. */
function asHeroVariant(value: unknown): HeroVariant | undefined {
  return value === 'default' || value === 'church' || value === 'funeral' || value === 'cleaning'
    ? value
    : undefined;
}
...
export default function HeroModule({ tenant, locale, content }: ModuleProps) {
  ...
  const variant = asHeroVariant(content.variant);
