        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-red-900/95">
          <div className="max-w-md rounded-lg bg-white p-8 text-center shadow-2xl">
            ...
            <div className="mb-4 border-t border-gray-200 pt-4">
              <p className="flex items-center justify-center gap-2 font-mono text-xs text-gray-500">
                <Lock className="h-3 w-3" />
                GDPR Article 44 Violation Prevented
              </p>
            </div>
            <Button onClick={handleReturnToSafeArea} className="bg-red-600 hover:bg-red-700">
          ...
          <CardContent className="flex flex-col items-center gap-4 py-8">
            ...
            <div className="text-center">
              ...
              <p className="mt-2 text-sm text-yellow-600 dark:text-yellow-400">
                GDPR Article 44 restricts cross-border data access. Your current context does not
                permit access to {targetCountry} data.
              </p>
            </div>
            <Button onClick={() => router.push(fallbackPath)}>Return to Dashboard</Button>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {showWatermark && userCountryCode && (
        <div className="pointer-events-none fixed bottom-4 right-4 z-50 opacity-50 transition-opacity hover:opacity-100">
          <div className="rounded-lg border-2 border-blue-600 bg-white p-3 shadow-lg">
            <div className="flex items-center gap-2">
              <span className="text-2xl">{countryInfo.flag}</span>
              <div>
                <p className="text-sm font-bold text-blue-900">{countryInfo.name}</p>
                <p className="text-xs text-gray-600">
                  Data Residency: {countryInfo.code.toUpperCase()}
                </p>
                <p className="flex items-center gap-1 font-mono text-xs text-green-600">
