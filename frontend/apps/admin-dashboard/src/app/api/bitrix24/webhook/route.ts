...
function recordFailure(country: string): void {
  ...
  if (cb.failureCount >= CIRCUIT_BREAKER_THRESHOLD) {
    ...
    console.warn(
      `[BITRIX24] Circuit breaker for ${country} OPENED after ${cb.failureCount} failures`
    );
  }
}
...
function addToRetryQueue(event: Bitrix24WebhookEvent, error?: string): void {
  const existingEntry = retryQueue.find(
    (e) => e.event.event === event.event && e.event.data.FIELDS?.ID === event.data.FIELDS?.ID
  );
...
async function processRetryQueue(): Promise<void> {
  ...
  const readyEntries = retryQueue.filter(
    (e) => e.nextRetry <= now && e.attempts < MAX_RETRY_ATTEMPTS
  );
  ...
  for (const entry of readyEntries) {
    try {
      ...
      if (entry.attempts >= MAX_RETRY_ATTEMPTS) {
        ...
        await createAuditLog('RETRY_EXHAUSTED', {
 ...
 */
async function createAuditLog(type: string, data: Record<string, unknown>): Promise<void> {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const country =
      body.country || body.data.residency || request.headers.get('x-bitrix24-country') || 'default';
