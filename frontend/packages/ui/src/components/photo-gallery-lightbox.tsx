...
export function Lightbox({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="h-auto max-h-[95vh] w-auto max-w-[95vw] border-0 bg-black/95 p-0">
        ...
        {/* Image counter */}
        <div className="absolute bottom-4 left-1/2 z-50 -translate-x-1/2 text-sm text-white">
        ...
        {currentPhoto.caption && (
          <div className="absolute bottom-12 left-0 right-0 px-4 text-center text-white">
            <p className="inline-block rounded bg-black/50 px-3 py-1 text-sm">
              {currentPhoto.caption}
            </p>
          </div>
        )}
